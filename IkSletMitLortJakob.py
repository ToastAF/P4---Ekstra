...
def update_plot(feature):
    ...
    for x in x_line:
        ...
        # Precompute common features used in multiple conditions
        ...
        # Extract features depending on the selected feature
        feature_dict = {
        ...
        if feature == 'Distortion':
            # Combining these features into a single measure for distortion
            feature_value = np.log1p(bandwidth + zcr + flatness)
        else:
            feature_value = feature_dict.get(feature, 0)
...
feature_selector = ttk.Combobox(right_frame, values=['Spectral Centroid', 'Spectral Rolloff', 'Spectral Contrast',
                                                     'Zero Crossing Rate', 'Spectral Bandwidth', 'Spectral Flatness',
                                                     'RMS Energy', 'Distortion'])
